...
const Navbar = () => {
  ...
  // ✅ Restore user state from localStorage on mount (survives page refresh)
  useEffect(() => {
  ...
  // ✅ Shared helper: save token + username, update state, redirect
  const handleAuthSuccess = (accessToken: string, userName: string) => {
    localStorage.setItem("access_token", accessToken); // ✅ key must be "access_token"
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // Email exists but was registered manually — ask user to use password
        if (res.status === 401) {
          toast({
        ...
        const userName = data.user?.name ?? userInfo.name ?? userInfo.email;
        toast({ title: `Welcome, ${userName}! 👋` });
        handleAuthSuccess(data.access_token, userName); // ✅ redirect here
      } catch (err: unknown) {
        const message =
