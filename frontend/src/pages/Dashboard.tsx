...
type RecentActivityItem = {
  merchant?: string;
  to_user?: string;       // backend field name
  status: string;
  amount?: number;
...
function normalizeArray<T>(payload: unknown): T[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const key of ["data", "items", "results", "activity"]) {
    ...
    const run = async () => {
      try {
        ...
        const [sRes, rRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/analytics/recent-activity`, { headers }),
        ]);
        ...
        if (rRes.ok) {
          ...
          // Filter out junk entries ($0 or no merchant)
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <StatCard
            ...
            valueClassName={fraudIsClean ? "text-primary" : "text-yellow-400"}
          />
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          <Link
            to="/split"
            className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground hover:opacity-90 transition glow-primary"
          >
            <Plus size={15} />
            Create Split
          </Link>
          <Link
            to="/cards"
            className="inline-flex items-center gap-2 rounded-xl border border-border bg-secondary/30 px-5 py-3 text-sm font-semibold text-foreground hover:bg-secondary transition"
          >
            <CreditCard size={15} />
            Manage Cards
          </Link>
...
// ─── StatCard ─────────────────────────────────────────────────────────────────
function StatCard({
  loading, icon, label, value, sub, valueClassName = "text-foreground",
}: {
  loading: boolean;
  ...
  return (
    <div className="glass gradient-border rounded-2xl p-6">
      <div className="flex items-center gap-3 mb-4">
        <div className="h-9 w-9 rounded-xl bg-secondary/60 border border-border/50 flex items-center justify-center">
          {icon}
        </div>
      ...
      {loading ? (
        <>
          ...
          <div className="h-3 w-20 rounded-lg bg-muted animate-pulse mt-2" />
        </>
      ) : (
        <>
          <div className={`text-3xl font-display font-bold tracking-tight ${valueClassName}`}>
            {value}
          </div>
  ...
  return (
    <div className="glass rounded-2xl px-5 py-4 flex items-center justify-between hover:bg-card/80 transition-colors">
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="font-semibold text-sm capitalize">{item.merchant || item.to_user || "Unknown Merchant"}</div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-bold text-sm">
          {formatMoney(item.amount_cents != null ? item.amount_cents / 100 : Number(item.amount ?? 0))}
        </span>
      </div>
    </div>
  );
